...
void AddInNative::addError(const wchar_t* descriptor)
{
	if (m_iConnect) {
		WCHAR_T* err = 0;
		WCHAR_T* descr = 0;
		const wchar_t* source = L"ScanPass";
...
void AddInNative::sendEvent(const wchar_t* wsMessage, const wchar_t* wsData)
{
	if (m_iConnect) {
		WCHAR_T* src = 0;
		WCHAR_T* msg = 0;
		WCHAR_T* data = 0;
		const wchar_t* source = L"ScanPass";
		::convToShortWchar(&src, source);
		::convToShortWchar(&msg, wsMessage);
		::convToShortWchar(&data, wsData);
		uint32_t wcode = ADDIN_E_VERY_IMPORTANT;
		m_iConnect->ExternalEvent(src, msg, data);
		delete[] data;
		delete[] msg;
		delete[] src;
	}
